...
def keras_debug(train_features,train_labels,test_features,test_labels,
    ...
    for i in range(5000):
        ...
        checkpoint = ModelCheckpoint(filepath = newpath + title + "_best.hdf5",
                monitor = 'val_loss', verbose = 0, 
                save_best_only = True, mode = 'min')
        train_step = model.fit(train_features, train_labels, epochs = 128, 
                validation_data=(validation_features, validation_labels),
                callbacks = [checkpoint],
                batch_size = train_labels.shape[0], verbose = 0)
            
        loss_value = train_step.history['loss']
        val_loss_value = train_step.history['val_loss']
        
        
        val_minloss_i=min(val_loss_value)
        
        train_loss = train_loss+loss_value
        validation_loss = validation_loss+val_loss_value
        
        if i==0:
            os.rename(newpath+title+"_best.hdf5",newpath+title+"_"+str(val_minloss_i)+".hdf5")
            val_minloss = val_minloss_i 
        elif val_minloss_i< val_minloss:
            os.rename(newpath+title+"_best.hdf5",newpath+title+"_"+str(val_minloss_i)+".hdf5")
            os.remove(newpath+title+"_"+str(val_minloss)+".hdf5")
            val_minloss = val_minloss_i 
        else:
            os.remove(newpath+title+"_best.hdf5")
    ...
    # plot_model(model ,to_file=newpath + title +'_'+str(val_minloss)+'_model.png')

    dataframe = pd.DataFrame({'train_loss':train_loss,'validation_loss':validation_loss})
    dataframe.to_csv(newpath+"/"+title+"_"+str(val_minloss)+'.csv',sep=',')
...
newpath = "./keras_debug_adm_final_128train_5/"
mkdir(newpath)
   

for N in [230,235]:
    for dropout in [0,0.2,0.3,0.5]:
         keras_debug(train_features,train_labels,test_features,test_labels,
                3,N,act = 'elu',dropout=dropout,newpath = newpath)
